...
export default function WorkoutRecord() {
  ...
  // useEffect para la carga de datos de sanity a traves de una groq query
  useEffect(() => {
    if (!workoutId) return;
  ...
  // Formatea la fecha en vez de esta forma "2000-7-22" → "Sabado, 22 de Julio de 2000"
  ...
  // Formatear duracion 3600 segundos → "60m 0s"
  ...
  // Formatear la hora "2024-01-15T14:30:00" → "14:30"
  ...
  //Funciona para contar el numero de series
  const getTotalSets = () => {
  ...
  // Funcion para contar el volumen total del entreno
  ...
  //Funcion de borrar entrenamiento en el frontend con alerta tanto en web como en movil
  const handleDeleteWorkout = () => {
  if (Platform.OS === 'web') { 
    if (window.confirm("¿Estás seguro de que quieres eliminar este entreno?")) {
    ...
    Alert.alert(
      "Elimina el entreno",
      "Estas seguro de que quieres eliminar este entreno?",
      [
        {
          text: "Cancelar",
          style: "cancel",
  ...
  //Funcion de borrado en el backend (sanity server)
  const deleteWorkout = async () => {
    if(!workoutId) return

    setDeleting(true)

    try {await fetch("/api/delete-workout", {
      method: "POST",
      body: JSON.stringify({workoutId})
    })
    router.replace("/(app)/(tabs)/history?refresh=true")
  }catch(error){
    console.error("Ha surgido un  error eliminando el entrenamiento:", error)
    Alert.alert("Ha surgido un error", "Vuelva ha intentarlo", [{text: "OK"},])
  } finally{
    setDeleting(false)
  }
}
    ...
    return (
      <SafeAreaView className="flex-1 bg-gray-50">
        <View className="flex-1 items-center justify-center">
          ...
          >
            <Text className="text-white font-medium">Vuelve</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
              ...
              {deleting ? (
                ...
                <>
                  <Ionicons
                    ...
                    color="#FFFFFF"
                  />
                  <Text className="text-white font-medium ml-2">Eliminar</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
          ...
          {workout.exercises?.map((exerciseData, index) => (
            ...
            >
              ...
              {exerciseData.sets && exerciseData.sets.length > 0 && (
                <View className="mt-4 mr-3 pt-4 border-t border-gray-100">
